...
if (!empty(isset($_GET['sef']))) {
  ...
  <head>
    <title><?php echo $kategoricek['kategori_ad'] ?>  Kategorisi</title>
  </head>
<?php } else {
  ?>
<head>
    <title><?php echo $kategoricek['kategori_ad'] ?>  Tüm Ürünler</title>
  </head>
  <?php } 
?>
		...
		<div class="col-md-9"><!--Main content-->
			...
			<div class="row prdct"><!--Products-->
				...
				if (isset($_GET['sef'])) {
					...
					$sorgu_urunsor=$db->prepare("SELECT * FROM urun where kategori_id=:kategori_id");
                    $sorgu_urunsor->execute(array(
                       'kategori_id' => $kategori_id
                   ));
                    $toplam_icerik_urun=$sorgu_urunsor->rowCount();
                    $toplam_sayfa_urun = ceil($toplam_icerik_urun / $sayfada);
